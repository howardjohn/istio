...
func makeClientVersions(t *testing.T, schemas collection.Schemas, versions sets.String) (model.ConfigStoreController, kube.CLIClient) {
	fake := kube.NewFakeClient()
	for _, s := range schemas.All() {
...
func createCRD(t test.Failer, client kube.Client, r resource.Schema, versions sets.String) {
	t.Helper()
