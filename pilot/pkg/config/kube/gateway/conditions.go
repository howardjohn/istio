...
type condition struct {
	...
	// the status inverted
	error *ConfigError
	// setOnlyIfReasonMatches, if enabled, will only set the condition if it is not yet present or set to this reason
	setOnlyIfReasonMatches string
}
...
func setConditions(generation int64, existingConditions []metav1.Condition, conditions map[string]*condition) []metav1.Condition {
	...
	for _, k := range condKeys {
		...
		if cond.setOnlyIfReasonMatches != "" {
			setter = func(conditions []metav1.Condition, condition metav1.Condition) []metav1.Condition {
				return kstatus.CreateCondition(conditions, condition, cond.setOnlyIfReasonMatches)
			}
		}
