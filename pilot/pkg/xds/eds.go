...
func (key EndpointBuilder) HashCode() uint32 {
	var result uint32
...
func (key EndpointBuilder) DebugCode() string {
...
func insertCache(key EndpointBuilder, value *any.Any) {
	cacheMutex.Lock()
...
func getCache(key EndpointBuilder) (*any.Any, bool) {
	cacheMutex.RLock()
...
func (s *DiscoveryServer) pushEds(push *model.PushContext, con *Connection, version string, edsUpdatedServices map[string]struct{}) error {
	...
	for _, clusterName := range con.Clusters() {
		...
		if marshalledEndpoint, f := getCache(builder); f {
