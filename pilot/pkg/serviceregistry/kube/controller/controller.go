...
func (c *Controller) onServiceEvent(_, curr *v1.Service, event model.Event) error {
	log.WithLabels("cluster", c.clusterID).Debugf("Handle event %s for service %s in namespace %s", event, curr.Name, curr.Namespace)
	defer func() {
		log.WithLabels("cluster", c.clusterID).Debugf("DONE Handle event %s for service %s in namespace %s", event, curr.Name, curr.Namespace)
...
func (c *Controller) HasSynced() bool {
	t := c.queue.HasSynced() || c.initialSyncTimedout.Load()
	if t {
		log.Infof("kube controller for %s really synced", c.opts.ClusterID)
	}
	return t
}
...
func (c *Controller) InstancesByPort(svc *model.Service, reqSvcPort int) []*model.ServiceInstance {
	...
	log.Errorf("howardjohn: ibp: %v", len(outInstances))
...
func (c *Controller) serviceInstancesFromWorkloadInstances(svc *model.Service, reqSvcPort int) []*model.ServiceInstance {
	...
	c.workloadInstancesIndex.ForEach(func(wi *model.WorkloadInstance) {
		...
		if selector.Match(wi.Endpoint.Labels) {
			...
			log.Errorf("howardjohn: got %v'nth instance for service: %v!", len(out), instance.Endpoint.Address)
			if instance != nil {
...
func (c *Controller) WorkloadInstanceHandler(si *model.WorkloadInstance, event model.Event) {
	...
	c.queue.Push(func() error {
		...
		log.WithLabels("cluster", c.clusterID).Errorf("howardjohn: workland instance %v %v", event, si.Name)
		defer func() {
			log.WithLabels("cluster", c.clusterID).Errorf("howardjohn: workland instance DONE %v %v", event, si.Name)
		}()
		...
		// find the services that map to this workload entry, fire off eds updates if the service is of type client-side lb
		allServices := c.services.List(si.Namespace, klabels.Everything())
		log.Errorf("howardjohn: got svc %v", len(allServices))
		if k8sServices := getPodServices(allServices, dummyPod); len(k8sServices) > 0 {
			log.Errorf("howardjohn: matched %v", len(k8sServices))
			for _, k8sSvc := range k8sServices {
				service := c.GetService(kube.ServiceHostname(k8sSvc.Name, k8sSvc.Namespace, c.opts.DomainSuffix))
				// Note that this cannot be an external service because k8s external services do not have label selectors.
				if service == nil || service.Resolution != model.ClientSideLB {
					// may be a headless service
					log.Errorf("howardjohn: service not alid %v", service)
					continue
				}
				...
				for _, port := range service.Ports {
					...
					for _, inst := range instances {
						...
						log.Errorf("howardjohn: fire off %v", inst.Endpoint.Address)
					}
				}
				// fire off eds update
