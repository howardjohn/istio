...
`,
		...
		RunE: func(c *cobra.Command, args []string) error {
			...
			if matcherProxyConfig {
				return configWriter.PrintMatcherListenerSummary()
			}
			switch outputFormat {
			case summaryOutput:
	...
	listenerConfigCmd.PersistentFlags().BoolVar(&matcherProxyConfig, "matcher", false, "Output matcher information")
