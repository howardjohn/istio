		...
		if a.Event == controllers.EventDelete {
			for oKey := range h.collectionState.inputs[iKey] {
				...
				if !f {
					...
					log.WithLabels("okey", oKey).Errorf("howardjohn: BUG, inconsistent")
					continue
				}
				e := Event[O]{
			...
			for key := range allKeys {
				...
				if newExists && oldExists {
					if controllers.Equal(newRes, oldRes) {
						// NOP change, skip
						continue
					}
		...
		for i, v := range h.objectRelations {
			...
			for _, item := range ev.Items() {
				...
				if d, f := v.dependencies[unnamed]; f {
					match := d.filter.Matches(item)
					log.WithLabels("match", match).Debugf("event for collection %v", unnamed.dtype)
					if match {
						ks.Insert(i)
						break
					}
	...
	for i := range ks {
		...
		if ii == nil {
			...
			for oKey := range h.collectionState.inputs[i] {
				...
				if !f {
					// Typically happens when O has multiple parents
					log.WithLabels("ikey", i, "okey", oKey).Errorf("howardjohn: BUG, inconsistent2")
					continue
				}
				e := Event[any]{
					Event: controllers.EventDelete,
...
func (h *manyCollection[I, O]) RegisterBatch(f func(o []Event[O])) {
	...
	objs := Map(h.List(metav1.NamespaceAll), func(t O) Event[O] {
		return Event[O]{
			New:   Ptr(t),
			Event: controllers.EventAdd,
		}
...
func (i *indexedManyCollection[I, O]) registerDependency(d dependency) bool {
	...
	if !i.h.dependencies.InsertContains(c.hash()) {
		log.Debugf("register manyCollection %T", c)
		i.h.mu.Unlock()
